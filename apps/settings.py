...
from .config import DJANGO_APPS, THIRD_PARTY_APPS, EDC_APPS, LIS_APPS, LOCAL_APPS
...
SOURCE_DIR = Path(__file__).ancestor(1)
MEDIA_ROOT = PROJECT_ROOT.child('media')
STATIC_ROOT = PROJECT_ROOT.child('static')
TEMPLATE_DIRS = (
    '{}/edc_project/edc/templates'.format(SOURCE_DIR),
)
STATICFILES_DIRS = ()
CONFIG_DIR = PROJECT_ROOT.child('bhp074')

# Key Path
# KEY_PATH = '{}/apps/keys'.format(PROJECT_DIR)
KEY_PATH = '/Volumes/bhp074/live_keys'
...
if platform.system() == 'Darwin':
    LAB_IMPORT_DMIS_DATA_SOURCE = 'DSN=%s;UID=%s;PWD=%s;DATABASE=%s;DRIVER=%s'.format(
        Eit.dsn, Eit.dsn_user, Eit.dsn_password, Eit.dsn_database, Eit.dsn_mac_driver)
else:
    LAB_IMPORT_DMIS_DATA_SOURCE = 'DSN=%s;UID=%s;PWD=%s;DATABASE=%s;DRIVER=%s'.format(
        Eit.dsn, Eit.dsn_user, Eit.dsn_password, Eit.dsn_database, Eit.dsn_driver)
VAR_ROOT = '/var'
