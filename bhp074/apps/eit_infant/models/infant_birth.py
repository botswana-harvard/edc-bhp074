...
from bhp074.apps.eit_maternal.models import MaternalConsent
...
class InfantBirth(BaseAppointmentMixin, BaseUuidModel):
    ...
    def __unicode__(self):
        return '%s [%s] %s %s' % (
            self.registered_subject.subject_identifier, self.initials, self.dob, self.get_gender_display())
    ...
    def safe_delete_appointment(self, code):
        ...
        for appointment in Appointment.objects.filter(
                registered_subject=self.registered_subject, visit_definition__code=code, visit_instance='0'):
    ...
    def delete_control_apps_post_save(self):
        maternal_subject = MaternalConsent.objects.get(
            registered_subject__subject_identifier=self.registered_subject.relative_identifier)
